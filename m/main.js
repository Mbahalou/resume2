...
function render_markdown(md_text, result_html_div = null, markedOptions = {}) {
    ...
    */
        ...
        tokens.map(function (token) {
            if (token.type === 'paragraph') {
                token.text.split(/((?:^|\n)&lt;summary&gt;[^]*?&lt;\/summary&gt;)/)
                    ...
                    .map(function (c) {
                        tokens2.push({type: 'paragraph', text: c})
                    });
            } else
                tokens2.push(token);
        });
        ...
        tokens2.map(function (token) {
            if (token.type === 'paragraph') {
                token.text.split(/((?:^|\n)&lt;(?:details\s*(?:(?: class=".*?")?(?: open)?|(?: open)?(?: class=".*?")?)|\/details)&gt;)/)
                    ...
                    .map(function (c) {
                        tokens.push({type: 'paragraph', text: c})
                    });
            } else
                tokens.push(token);
        });
        ...
        patterns_and_replacements.forEach(
            function (pattern_and_replacement) {
                ...
                if (use_placeholder) {
                    ...
                    replacement = pattern_and_replacement['replacement'];
    ...
    html_div.find('img').css("cursor", "pointer")
    html_div.find('img').click((e) => {
        const img = e
        $("#img-modal").modal({
            close: '.close.icon',
            onShow: function () {
    ...
    html_div.append(`<div class="ui large modal" id="img-modal">
            <i class="close icon"></i>
            <div class="image content">
                <img class="image" style="width: 100%" src=""/>
            </div>
            <div class="actions">
                <div class="ui cancel button">بازگشت</div>
            </div>
        </div>`)
